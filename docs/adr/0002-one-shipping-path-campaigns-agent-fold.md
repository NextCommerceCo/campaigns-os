...
- The pin checks (`unpinned`, `conflicting_pin`, `--force` recorded on the journal entry) are introduced by a pull request after step 4 and before step 5, tracked as campaigns-os#466; the trial precondition above holds with or without that issue.
