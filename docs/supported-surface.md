...
| Package export `./text-safety` | `singleLineField` and `singleLineDetail`: flatten a value this toolkit did not author (a run id, a target path, a quoted loader message) to one line with no control characters, and in the detail case no Markdown, before it is rendered into a single-line notice. | Pure string functions, no imports, no I/O. Added at surface 1.27.0. The escape set may widen; a value that was already safe stays unchanged. |
