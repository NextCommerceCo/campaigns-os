...
| CLI commands: `start`, `prepare-build`, `build`, `polish`, `checkpoint`, `doctor`, `standardize`, `standardization-report`, `qa`, `findings`, `run-record`, `run` | Scriptable entry points (this is the argv surface Campaigns Agent's remit fixture pins). `polish capture` owns page-load evidence production. `checkpoint waive` currently accepts four registered gates: `page_kit.store_profile`, `page_kit.sdk_version`, `polish.hidden_eager_media`, and `built_output.upsell_selector_scope`. Within Polish, only the broader Source Freshness waiver remains on its existing report lane; theme and QA decisions also retain their existing lanes. | Additive commands bump `surface_version` in the same PR. Subcommands, registered gates, and flags may grow freely beneath a listed command. Renaming or removing one fails the gate. Do not infer support for an unregistered checkpoint from the top-level command. |
...
| Package exports `./reconcile-plan`, `./reconcile-normalize`, `./reconcile-diff` | Campaign reconciliation: CampaignSpec to observation descriptors, raw Admin API envelopes to canonical observed state, and exact-only comparison producing a closed-vocabulary verdict matrix. Pure functions with no network transport and no credential handling. The normalizer is also the redaction boundary — the campaign `api_key` is dropped there, so no downstream code can emit it. | Comparison rules are versioned in `docs/reconcile-comparison-matrix-v0.md` and hashed into every report. A page may reference one package at several quantities (a bundle picker), so the compared unit is a selection entry and many entries may bind to one live package. Rows this source cannot yet observe (Offers, campaign shipping methods) are classed `unsupported` and counted, never dropped. |
| Contract `contracts/reconcile-comparison-matrix.v0.json` | The reconciliation comparison rules as data: field paths, normalization, verdict vocabulary, the selection-entry package model, and coverage rules. This file is the authority; `docs/reconcile-comparison-matrix-v0.md` explains it but does not define it. | Hashed entry — changes require a `surface_version` bump in the same change. `scripts/check-reconcile-matrix.mjs` fails the build if the modules and this contract disagree, so the `matrix_hash` embedded in every report certifies enforced rules rather than prose. |
