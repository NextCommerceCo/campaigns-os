...
| CLI commands: `start`, `prepare-build`, `build`, `checkpoint`, `doctor`, `standardize`, `standardization-report`, `qa`, `findings`, `run-record`, `run` | Scriptable entry points (this is the argv surface Campaigns Agent's remit fixture pins). `checkpoint` is a staged registry: `checkpoint waive` currently accepts `page_kit.store_profile` and `page_kit.sdk_version`. Source Freshness, polish, theme, and QA keep their separate waiver lanes until explicitly registered. | Additive commands bump `surface_version` in the same PR. Subcommands, registered gates, and flags may grow freely beneath a listed command. Renaming or removing one fails the gate. Do not infer support for an unregistered checkpoint from the top-level command. |
