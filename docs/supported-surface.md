...
| CLI commands: `start`, `prepare-build`, `build`, `doctor`, `standardize`, `standardization-report`, `qa`, `findings`, `run-record`, `run` | Scriptable entry points (this is the argv surface Campaigns Agent's remit fixture pins). | May gain subcommands and flags freely. Renaming or removing one fails the gate. |
