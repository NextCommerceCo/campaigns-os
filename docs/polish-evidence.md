...
| `polish.hidden_eager_media.capture_incomplete` | One or more required route/viewport measurements failed completeness. Repair the named capture problem and recapture; this is not waivable. The blocked checkpoint carries `measurement` (the recomputed `status`, counts, and the `missing[]`, `duplicate[]`, `unexpected[]` and `incomplete[]` cells with their `problem_codes[]`), and the QA verdict's `polish.hidden_eager_media` assertion projects it as `evidence.measurement` (bounded to the full 128-route, two-viewport matrix; anything past that is counted in `omitted_cell_count`), so the failing route, viewport and problem code are readable from the verdict alone. `dependency_request_failed` names a document, first-party, or script/stylesheet/media failure; a `cross_origin_request_failed` warning alone never produces this code. |
