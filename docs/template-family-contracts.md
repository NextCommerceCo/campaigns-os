...
| --- | --- | --- | --- | --- | --- | --- |
| `apollo` | landing, presell, checkout, upsell, receipt | Tiered selector with `data-next-bundle-selector` / `data-next-bundle-card` and product/classic/tile card dispatch | Selector-area `bump-check03.html` plus form-section bump variants; remove without prepurchase packages | Bundle stepper, tier pills, and tier cards; visible price above accept CTA | Include exists but is not default; strip unless CampaignSpec maps `exit_intent` / `promo_code_input` | Apollo checkout shell, selected bundle card/tile, visible prices, branded logo |
| `apollo-mv-single-step` | landing, presell, checkout, upsell, receipt | Single-page configurable MV selector with `variant_slots` and slot personalization support | Optional selector-area/form-section bumps; sync through the declared MV package/product contract | MV and bundle upsells; visible price rows | Include exists but is not default; strip unless CampaignSpec maps an offer/code | Apollo MV checkout shell, slot anchors, selected card state, visible prices, branded logo |
