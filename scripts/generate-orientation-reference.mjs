...
function buildCanonicalizationFixture() {
  ...
  const section = parseChangelogSections(inputChangelog).find((candidate) => candidate.section_id === "1.18.0");
  ...
  return {
    schema_version: "campaigns-os-release-ledger-canonicalization-example/v1",
    entry_sha256: {
...
export function renderReference({ orientationSchema, ledgerSchema, policy, reasonCodes, limits, surface, fixtures, canonicalizationFixture }) {
  ...
  push(
    ...
    "4. Serialize strings, finite numbers, booleans, and `null` with ECMAScript `JSON.stringify`. The ledger schema admits only JSON values. If an in-memory caller nevertheless supplies a present `undefined` value, it canonicalizes as `null`; an absent key remains absent.",
