...
export const CANONICALIZATION_FIXTURE_PATH = "contracts/fixtures/orientation/canonicalization/v1.json";
...
function buildCanonicalizationFixture() {
  const example = {
    ...
    id: "RL-9001",
    date: "2026-09-02",
    changelogSection: "0.0.0+agent.9001",
  };
  const inputChangelog = [
    "# Changelog",
    "",
    `## [${example.changelogSection}] - ${example.date}`,
    "",
    "### Added",
    "",
  ...
  const section = parseChangelogSections(inputChangelog).find((candidate) => candidate.section_id === example.changelogSection);
  if (!section) throw new Error("canonicalization example changelog section was not parsed");
  const entryWithoutDigest = {
    ...
    changelog_section: section.section_id,
    changelog_sha256: section.body_sha256,
    agent_impact: "A consumer can reproduce ledger digests independently.",
    compatibility: "compatible",
    migration: "none",
    changes: [
      {
        summary: "Publish the digest rule and a self-check vector.",
  ...
  const inputEntry = { ...entryWithoutDigest, entry_sha256: entryHash(entryWithoutDigest) };

  return {
    fixture_version: "campaigns-os-release-ledger-canonicalization-example/v1",
    entry_sha256: {
      input_entry: inputEntry,
      canonical_utf8: canonicalEntryJson(inputEntry),
      digest: entryHash(inputEntry),
    },
    changelog_sha256: {
      input_utf8: inputChangelog,
      section_id: section.section_id,
      section_body_utf8: section.body,
      digest: section.body_sha256,
    },
  };
}
...
export function renderReference({ orientationSchema, ledgerSchema, policy, reasonCodes, limits, surface, fixtures, canonicalizationFixture }) {
  ...
  push(
    ...
    "is generated by the same implementation that validates the ledger; CI reproduces every byte and digest before publishing this reference.",
    "A consumer can therefore verify an independent implementation without reading anything under `scripts/**`.",
    "Its `fixture_version` identifies the self-check vector shape; it is not a ledger or orientation payload schema ID.",
    "",
    "### `entry_sha256`",
    "",
    "1. Start from the parsed ledger-entry object and remove exactly the root property named `entry_sha256`. A missing property stays missing; no other property at any depth is excluded.",
    "2. Serialize objects with their own enumerable string keys sorted in ascending ECMAScript default string order (UTF-16 code-unit order). Encode each key with ECMAScript `JSON.stringify`, then `:`, then its canonical value; join members with `,` and add `{` / `}` with no whitespace.",
    "3. Serialize arrays in their existing order, joining canonical elements with `,` and adding `[` / `]` with no whitespace. Array order is never sorted.",
    "4. Serialize strings, numbers, booleans, and `null` with ECMAScript `JSON.stringify`. Canonicalization operates on the parsed ECMAScript value: `-0` becomes `0`, and integers outside the safe-integer range have already taken their parsed `Number` value. The ledger schema admits only JSON numbers, so `NaN`, positive or negative `Infinity`, and other non-JSON numeric inputs are outside the contract; if an in-memory caller nevertheless supplies one, the current canonicalizer emits `null`, matching `JSON.stringify`. A present `undefined` value likewise canonicalizes as `null`; an absent key remains absent.",
    "5. Compute SHA-256 over the exact UTF-8 bytes of that canonical string and encode the digest as lowercase hexadecimal.",
    "",
    "Worked canonical byte string:",
    "",
    "```json",
    canonicalizationFixture.entry_sha256.canonical_utf8,
    "```",
    "",
    `Result: \`${canonicalizationFixture.entry_sha256.digest}\``,
    "",
    "### `changelog_sha256`",
    "",
    "1. Split `CHANGELOG.md` on LF (`\\n`) into lines. A section heading is a whole line matching `^## \\[([^\\]]+)\\] - (\\d{4}-\\d{2}-\\d{2})\\s*$`; capture group 1 is the section id and group 2 is its date.",
    "2. A section body starts with its heading line and ends immediately before the next matching heading, or at end-of-file when there is no next heading.",
    "3. Rejoin those lines with LF, then remove the single contiguous run of whitespace matching `\\s+$` from the end of the whole section. Interior whitespace and line endings are otherwise bytes of the body; no newline is appended.",
    "4. Compute SHA-256 over the exact UTF-8 bytes of the resulting body and encode the digest as lowercase hexadecimal.",
    "",
    ...
    "",
    "```markdown",
    canonicalizationFixture.changelog_sha256.section_body_utf8,
    "```",
    "",
    `Result: \`${canonicalizationFixture.changelog_sha256.digest}\``,
    "",
  );
...
export function generate() {
  ...
  const validate = ajv.compile(orientationSchema);
  const validateLedgerEntry = ajv.compile({ $ref: `${ledgerSchema.$id}#/$defs/entry` });
  const errors = [];
  ...
  if (!validateLedgerEntry(canonicalizationFixture.entry_sha256.input_entry)) {
    errors.push(
      `canonicalization entry: ${validateLedgerEntry.errors.map((e) => `${e.instancePath} ${e.message}`).join("; ")}`,
    );
  }
  if (errors.length) throw new Error(`generated orientation fixtures do not validate:\n  - ${errors.join("\n  - ")}`);
