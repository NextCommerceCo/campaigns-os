...
- For `landing` and `presell` pages, prefer the prepared source HTML when `source_html.pages[].path` points at a real standalone page. Preserve the design/content through a passthrough page-kit layout, inject the SDK loader/config as needed, and repoint CTAs into the CampaignSpec flow. Treat `source_html.pages[].path` as source provenance and `source_html.pages[].page_kit` as the Page Kit target file, route, CPK `page_type`, and frontmatter projection.
