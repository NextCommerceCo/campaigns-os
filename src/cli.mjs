...
function prepareBuild(args, options = {}) {
  ...
  const packet = {
    ...
    qa: {
      ...
      test_order_policy_notes: "Default: SDK-driven test orders are not fired until the deployed domain is allowlisted and the operator-approved order count/path depth is clear.",
    },
...
Run the browser install once after install/update before --browser or --test-order. Test-order proof must exercise the deployed campaign through the Campaign Cart SDK with the browser typed-card flow. Do not create hand-built backend API orders as launch proof. Only fire test orders when test_orders_allowed=true, sandbox_test_card_confirmed=true, the deployed domain is allowlisted, and the operator-approved order count/path depth is clear; the standard sandbox card credentials do not need merchant-specific routing confirmation. Use --test-order checkout/decline/accept/both/full, or an explicit accept/decline path such as accept-decline-accept, with --allow-test-orders and --sandbox-test-card-confirmed; then click rendered SDK upsell accept/decline controls for upsell proof. Use the operator-provided shared safe inbox as the customer email when one is available. For multi-market campaigns, verify at least one non-default currency/country path: currency display, shipping method names/prices, payment methods, and market-specific copy. Summarize blockers, warnings, and remaining launch risks.`;
}
      ...
      return {
        ...
        actions: actions.length
          ...
          : blockedStages.includes("test-orders")
            ? ["Confirm test-order permission, domain allowlist, and order count/path depth before firing checkout mutations."]
