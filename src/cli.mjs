...
Run the browser install once after install/update before --browser or --test-order. Test-order proof must exercise the deployed campaign through the Campaign Cart SDK with the browser typed-card flow. Do not create hand-built backend API orders as launch proof. Only fire test orders when test_orders_allowed=true, sandbox_test_card_confirmed=true, and the deployed domain is allowlisted. Use --test-order checkout/decline/accept/both/full, or an explicit accept/decline path such as accept-decline-accept, with --allow-test-orders and --sandbox-test-card-confirmed; then click rendered SDK upsell accept/decline controls for upsell proof. For multi-market campaigns, verify at least one non-default currency/country path: currency display, shipping method names/prices, payment methods, and market-specific copy. Summarize blockers, warnings, and remaining launch risks.`;
}
