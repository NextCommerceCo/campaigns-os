...
function prepareBuild(args, options = {}) {
  ...
  const packet = {
    ...
    qa: {
      ...
      test_order_policy_notes: "Test Orders use global test cards that bypass the gateway and create no transactions. Run them any time with `qa run --test-order common` (3-5 shape sample) or `--test-order full` (every permutation). These flags are informational, not a permission gate.",
    },
...
export function validateSpecStoreProfile(spec, errors, warnings, ready) {
  ...
  if (isNonEmptyString(storeUrl) && looksLikePlaceholderStoreUrl(storeUrl)) {
    addIssue(
      ...
      `CampaignSpec campaign.store_url "${storeUrl}" looks like a local/placeholder store, not a live storefront. A real shopper cannot transact against it. Set the merchant's production store_url before launch.`
    );
...
Run the browser install once after install/update before --browser or --test-order. Test-order proof must exercise the deployed campaign through the Campaign Cart SDK with the browser typed-card flow. Do not create hand-built backend API orders as launch proof. Test Orders use global test cards that bypass the payment gateway and create no transactions, so they are safe to run any time and need no permission flags, packet policy, or merchant setup. Use --test-order common for the default 3-5 shape sample (checkout, plus accept/decline when there are upsells), an explicit path such as accept-decline-accept for a targeted matrix, or --test-order full for every permutation; then click rendered SDK upsell accept/decline controls for upsell proof. Reuse one test customer email via --test-email or CAMPAIGNS_OS_QA_TEST_EMAIL (a real monitored inbox in internal runs) so repeated QA does not litter the customer list. For multi-market campaigns, verify at least one non-default currency/country path: currency display, shipping method names/prices, payment methods, and market-specific copy. Summarize blockers, warnings, and remaining launch risks.`;
}
...
function printNextTinyPrompt(result, args) {
  ...
  console.log("Browser QA needs a deployed base URL. Typed-card test orders run any time with --test-order common (test cards bypass the gateway; no approval needed).");
