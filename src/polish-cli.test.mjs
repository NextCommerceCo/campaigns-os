...
test("polish capture text reports unavailable resource types when every value is filtered out", () => {
  const output = formatPolishCaptureText({
    status: "ready",
    measurement: {
      status: "complete",
      ...
      warnings: [{
        ...
        failed_origins: ["https://attribution.example.invalid"],
        failed_origin_count: 1,
        resource_types: ["script", "stylesheet", "image"],
      }],
    },
    checkpoint: { code: "polish.hidden_eager_media.pass", findings: [], required_actions: [] },
    observed_findings: [],
  ...
  assert.match(output, /Resource types: unavailable$/m);
  assert.doesNotMatch(output, /Resource types: *$/m);
  assert.doesNotMatch(output, /script|stylesheet|image/);
});
